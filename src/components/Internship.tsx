...
const InternshipForm = ({ onClose, defaultCourse }) => (
  <div className="fixed inset-0 z-[99999] flex flex-col sm:flex-row items-center justify-center bg-black bg-opacity-50">
    <div className="bg-white p-4 lg:p-5 rounded-2xl lg:rounded-3xl shadow-lg relative w-[90vw] lg:w-[700px] max-h-[88%] lg:max-h-[72vh] h-auto lg:min-h-auto lg:h-auto">
      {/* Close Button */}
      <button
        ...
        <div className="px-5 sm:px-6 md:px-10 lg:px-16 pt-12 pb-16 lg:py-16 w-full bg-[#021734] hover:shadow-[0px_0px_0px_5px_#2c4464] hover:scale-[0.9935] rounded-[40px] lg:rounded-[70px] transform transition-all ease-linear duration-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-7 lg:gap-6">
            {videoData.map((data, index) => (
              <div
                key={index}
                className="bg-white border rounded-2xl hover:scale-[1.002] hover:shadow-[3px_3px_0px_3px_#2c4464] p-4 flex flex-col group transition-all transform ease-linear duration-200"
              >
                <img
                  src={data.thumbnail}
                  alt="Video Thumbnail"
                ...
                <button
                  ...
                  className="mt-auto inline-flex items-center font-medium text-[15px] text-[#4288e3] hover:opacity-90 cursor-pointer group"
                >
                  Register Now
                  <img
                    src={Code}
